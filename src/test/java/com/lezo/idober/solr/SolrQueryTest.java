...
public class SolrQueryTest {
	...
	public void testQueryKeyWord() throws Exception {
		...
		solrQuery.set("q", "猩求崛起");
		solrQuery.set("q", "猩求崛起");
		solrQuery.setStart(0);
		solrQuery.setRows(1000);
		solrQuery.set("spellcheck", "true");
		QueryResponse response = server.query(solrQuery);
		SpellCheckResponse checkResp = response.getSpellCheckResponse();

		System.err.println(JSON.toJSONString(response.getResults()));
		System.err.println("getCollatedResults:" + JSON.toJSONString(checkResp.getCollatedResults()));
		System.err.println("getSuggestions:" + JSON.toJSONString(checkResp.getSuggestions()));
		System.err.println("getSuggestions:" + JSON.toJSONString(checkResp.getSuggestionMap()));
	}
