...
public class TimeController extends BaseController {
	...
	@ResponseBody
	@RequestMapping(value = { "task/share" }, method = RequestMethod.GET)
	public String createShareTasks() throws Exception {
		int start = 0;
		int limit = 100;
		int total = 0;
		String sMark = "bdp-uk;";
		while (true) {
			...
			for (SolrDocument doc : docList) {
				Object idObject = doc.getFieldValue("id");
				if (idObject == null) {
					continue;
				}
				String sId = idObject.toString();
				int index = sId.indexOf(sMark);
				if (index != 0) {
					log.warn("bad id:" + sId);
					continue;
				}
				String sUK = sId.substring(sMark.length());
				JSONObject taskObj = new JSONObject();
				taskObj.put("type", "bdp-share");
				taskObj.put("url", "");
				taskObj.put("level", 1000);
			...
			TaskUtils.createTasks(taskArray);
			total += taskArray.size();
			log.info("create task,share:" + taskArray.size());
			if (docList.size() < limit) {
				break;
			}
			start += docList.size();
