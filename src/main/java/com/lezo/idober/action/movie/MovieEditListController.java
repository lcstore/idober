...
public class MovieEditListController extends BaseController {
	...
	public ModelAndView listFix(ModelMap model, HttpServletRequest request)
			throws Exception {
		return listFix(1, model, request);
	}

	@SuppressWarnings("unchecked")
	@RequestMapping(value = { "list/{curPage}.html" }, method = RequestMethod.GET)
	public ModelAndView listFix(@PathVariable("curPage") Integer curPage, ModelMap model, HttpServletRequest request)
			throws Exception {
		...
		solrQuery.set("q", "(release:[NOW-" + beforeDay + "DAY/DAY TO NOW/DAY+30DAY])");
